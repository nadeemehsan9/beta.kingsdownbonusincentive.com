...
export default function ViewAllUserStates() {
    ...
    const getResultData = async () => {
      try {
        if (values.fieldtype === "" && values.searchval === "") {
          ...
          const response = await AdminListService.getNewLimitUserStateList(
            newLimit
          );
          ...
          const response = await AdminListService.getSearchUserStateList(
            values.fieldtype,
            values.searchval,
            newLimit
          );
  ...
  const changePagination = (e) => {
    ...
    const getResultData = async () => {
      ...
      try {
        const response = await AdminListService.getPaginationUserStateList(
          pageNo,
          limit
        );
  ...
  const getStateData = async () => {
    setLoading(true);
    try {
      const response = await AdminListService.getUserStateList();
      let resultData;
              ...
              items.map((el, index) => (
                <tr key={el.id}>
                  ...
                  <td className="add-edit-delete-inline text-start">
                    <Link
                      to={"/admin/edit-user-state/" + el.id}
  ...
  const delData = async (delId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          await AdminListService.deleteUserState(delId, adminId);
          setLoading(false);
            ...
            <div className="row">
              <div className="col">
                <div className="manage-heading-2">
                  <h2>
                    All States <span>[{totalResults}]</span>
                  </h2>
                </div>
                      ...
                      <div className="row mb-3">
                        <div className="col-lg-3">
                          ...
                          >
                            ...
                            <option value="3">State Code</option>
                          </select>
                          {errors.fieldtype && touched.fieldtype ? (
