...
export default function ViewRSAs() {
  ...
  const changeSelection = () => {
    ...
    $(".export-selection").attr(
      "href",
      `https://beta.claims-api.elitestacks.com/v1/export-selected-rsa/${selectedRecords}?time=${time}`
    );
  ...
  const allSelect = async () => {
    ...
    $(".export-selection").attr(
      "href",
      `https://beta.claims-api.elitestacks.com/v1/export-selected-rsa/${allSelections}?time=${time}`
    );
  };
  ...
  const getResultData = async (e) => {
    ...
    try {
      if (values.fieldtype === "" && values.searchval === "") {
        const response = await AdminListService.getSearchRsaList(
          "",
          "",
          e,
          limit,
        ...
        const response = await AdminListService.getSearchRsaByDate(
          values.fieldtype,
          values.searchval,
          limit,
          1
  ...
  const deactiveData = async (delId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          await AdminListService.deactiveParticipantsData(adminId, delId);
          setLoading(false);
  ...
  const searchMethod = async () => {
    ...
    try {
      const response = await AdminListService.getSearchRsaStoreList(
        limit,
        st_name,
        selectedCol,
        selectedVal,
  ...
  useLayoutEffect(() => {
    const getResultData = async () => {
      setLoading(true);
      try {
        const response = await AdminListService.getRsaList();
        let resultData;
        if (response.status === 206) {
          resultData = response.data.response;
          setResultData(resultData);
              ...
              items.map((el, index) => (
                <tr key={el.id}>
                  ...
                  <td className="add-edit-delete-inline">
                    ...
                    >
                      Deactive <i className="bi bi-person-x-fill"></i>
                    </button>
                    <Link
            ...
            <div className="row">
              <div className="col">
                <div className="manage-heading-2">
                  <h2>
                    View RSA <span>[{totalResults}]</span>
                  </h2>
                </div>
                          ...
                          <div className="col-lg-1">
                            <li className="dropdown">
                              ...
                              <ul className="dropdown-menu">
                                ...
                                <li>
                                  <a
                                    href={`${process.env.REACT_APP_API_Link}export-all-rsa?time=${time}`}
