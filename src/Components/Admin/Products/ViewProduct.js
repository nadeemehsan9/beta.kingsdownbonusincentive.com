...
export default function ViewProduct() {
  ...
  const getResultData = async (e) => {
    ...
    try {
      if (values.fieldtype === "" && values.searchval === "") {
        const response = await AdminListService.getSearchProductList(
          "",
          e,
          limit,
          1
    ...
    const getResultData = async () => {
      try {
        if (values.fieldtype === "" && values.searchval === "") {
          ...
          const response = await AdminListService.getNewLimitProductList(
            newLimit
          );
  ...
  const changePagination = (e) => {
    ...
    const getResultData = async () => {
      ...
      try {
        const response = await AdminListService.getPaginationProductList(
          values.fieldtype,
          values.searchval,
          pageNo,
          limit
  ...
  useLayoutEffect(() => {
    const getResultData = async () => {
      setLoading(true);
      try {
        const response = await AdminListService.getProductList();
        let resultData;
        if (response.status === 206) {
          resultData = response.data.response;
          setResultData(resultData);
            ...
            <div className="row">
              <div className="col">
                <div className="manage-heading-2">
                  <h2>
                    All Products <span>[{totalResults}]</span>
                  </h2>
                </div>
